...
To do this calculation, the proposer will need to do the following steps:
	...
	3. Use the FITS file from step 3 as input into this :download:`Pressure Profile Simulation Notebook </_static/mustang2_documents/ipynb/sensitivity_calculations/M2_FitPressureProfile.ipynb>`, to simulate the product (pressure profile and/or estimated mass). NB the notebook assumes a particular Y-M relation to estimate mass. We prefer these numbers be relayed in the technical justification, even if the proposer wishes to estimate mass via another method.
	4. Iterate as needed. If the product does not meet the scientific goals of the proposer (for example say the error bars are too large), play around with the integration time in ``M2_SimObs_A10.ipynb`` until you achieve the desired result. When you achieve a product that you are happy with, the total time contained in the ``times``  variable in the ``M2_SimObs_A10*.ipynb`` will be your integration time request (don't forget to add a factor of two for overheads). 
...
------------------------------------
Let's say that you have a previous observation of the same source or a similar source (you can find those observations by following the instructions in :ref:`mustang2_find_src`. You will have :math:`t_1` which is the number of hours the source was observed and the RMS achieved in that observation, :math:`\sigma_1`.
