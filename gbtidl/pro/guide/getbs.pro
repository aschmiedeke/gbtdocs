...
;
;   * In beam switched data, there are two beams in each scan, some 
...
;     Jy are requested via the units keyword, then :idl:pro:`dcsetunits`  is 
;     used to convert to the desired units.
;
;   * Within each integration, there are spectra identified as
...
; described in :idl:pro:`donod`  The other 4 Tsys values
...
;       Tracking feed number. Defaults to the lowest value with data 
;       after determining the value of ifnum if not supplied by the
;       user and using any value of plnum supplied by the user.  
;   bswitch : in, optional, type=integer
;       determines which beamswitch positions are used.  0=both, 1=ref,
;       2=sig (default 0)
...
;       tsys {in}{optional}{type=float} Tsys at zenith, this is converted
;       to a Tsys at the observed elevation.  If not suppled, the Tsys for
;       each integration is calculated as described elsewhere.
...
;       smooth factor for reference spectrum, defaults to 1 (no 
;       smoothing).
...
;       When set, the normal status message on successful completion is
;       not printed.  This will not have any effect on error messages. 
;       Default is unset.
;   keepints : in, optional, type=boolean
;       When set, the individual integrations are saved to the current
;       output file (fileout).  This is ignored if a specific integration
;       is requested using the intnum keyword.  Default is unset.
...
;       this specific file.  Default is all files currently opened.
...
;       integrations were processed (and possible saved to the output file
...
;       processed. This keyword is primarily of use when using getbs within
