...
pro nregion, regions
    ...
    if (nregions gt 100) then begin
        message, 'No more than 100 regions can be specified, ignoring all regions beyond that limit',/info
        nregions = 100
    endif
    ...
    order = sort(start)
    thisregion = 0
    !g.regions[0,0] = start[order[0]]
    !g.regions[1,0] = stop[order[0]]
    for i=1,(nregions-1) do begin
        inRegion = order[i]
        if (start[inRegion] le !g.regions[1,thisregion]) then begin
            ; this overlaps previous
            if (stop[inRegion] gt !g.regions[1,thisregion]) then begin
                ; new stop, else fully contained in previous region
                !g.regions[1,thisregion] = stop[inRegion]
            endif
        endif else begin
            ; new region
            thisregion += 1
            !g.regions[0,thisregion] = start[inRegion]
            !g.regions[1,thisregion] = stop[inRegion]
        endelse
    endfor

   !g.nregion = thisregion+1

    ; and set the rest to -1
    if (thisregion lt 99) then begin
        !g.regions[0:1,(thisregion+1):99] = -1
    endif
end
