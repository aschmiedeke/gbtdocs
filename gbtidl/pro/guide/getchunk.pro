...
;
; <p>It is more efficient to retrieve multiple data containers in one 
...
; by the user. 
;
; <p>Note that it is the responsibility of the caller to explicitly
; free the pointers used in this array of data containers.  Simply use
; <a href="../toolbox/data_free.html">data_free</a> as shown in the examples below.
;
; <p>The usual data selection operations can be specified in this
; function. 
;
; <p><b>There is no protection against running out of memory.</b>  If
...
;
; @keyword count {out}{optional}{type=integer} An output value giving the 
...
;
; @returns An array of data containers.  If no data satisfy the
