...
;
; :Params:
;   x : in, optional, type=numerical vector
;       The x values to be plotted. If omitted, then x is assumed to be
;       channels starting at 0 and going through n_elements(y) - 1. When
;       supplied, it must have the same number of elements as y. Unless
;       chan is specified, this is assumed to be in the same units as the
;       existing x axis.
;
;   y : in, required, type=numerical vector
...
;       index is cleared, subsequent indexes are renumbered - i.e. there 
;       are never any gaps in in index number.
;
...
;       less likely to conflict with user-defined idstrings. The default is ''.
;
;-
