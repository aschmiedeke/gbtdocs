...
;
; The VSP* values should be taken from a GBTIDL data container,
...
; :math:`spur_j = (j - VSPRVAL)*VSPDELT + VSPRPIX`, where j is an integer 
; that goes from 0 through 33.
; The VSP* values are double precision floats but the spur_j is
; assumed to be an integer.  Conversion to an integer is done using
; the IDL round function.
;
; NCHAN is used to determine which spur_j values should be
...
;
; The value at j=16, the center ADC spur, is not included in the
...
;
; These spurs do not include the center spur.  That spur always
...
;
; :Params:
;   vsprval : in, required, type=double
;       The spur reference value.
;   
;   vsprpix : in, required, type=double
;       The channel number (pixel) corresponding to the spur reference
;       value (vsprval).
;   
;   vspdelt : in, required, type=double
;       The channel increment between adjacent spur numbers (j to j+1).
; 
;   nchan : in, required, type=integer
;       The number of channels to use in determining which spur_j values 
;       to return.
; 
; :Keywords:
;   docenterspur : in, optional, type=boolean
;       When set, the center ADC spur will be included in the array of 
;       returned channel numbers.  The center ADC spur is normally 
;       interpolated across by sdfits and is typically not needed when 
;       reflagging or interpolating across the spurs in GBTIDL. The 
;       default behavior is to not return that channel in the list of 
;       spur channel numbers.
;
;   count : out, optional, type=integer
;       The number of valid VEGAS spur channels returned.
;
; :Returns:
;   an array listing the channel numbers associated with all of
