...
;   dc : out, required, type=spectrum
;       The resulting average.
;
; :Keywords:
;   noclear : in, optional, type=boolean, default=F
;       When set, the contents of the global accum buffer are not 
;       cleared.  This is useful when you want to see what the current
;       average is but also plan on continuing to add data to that 
;       average.  If this is not set, you would need to restart the
;       accumulation to average more data.
;
;   quiet : in, optional, type=boolean, default=F
;       Normally, accumave announces how many spectra were averaged.
;       Setting this turns that announcement off. This is especially
;       useful when multiple accum buffers are used within a procedure.
;
;   wtarray : out, optional, type=float
;       Optionally return the weight array (same length as the data
;       in the dc parameter) used in constucting the average.
; 
;   count : out, optional, type=integer
;       The number of spectra that have been averaged.  Returns -1 on
;       an error. If count=0 then the other output arguments (dc and
;       wtarray) are unchanged by this procedure.
;
; :Examples:
; 
;   .. code-block:: IDL
