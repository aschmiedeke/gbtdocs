...
;
; <p>Takes the data values in the given data container and 
...
;
; <p>The <b>dc</b> argument is altered by this operation.  The data
...
; ends of that data.
;
; <p>Interpolation is done using INTERPOL or internally (nearest).  
...
;
; <p>This only works with spectrum data containers.
;
; @param dc {in}{out}{required}{type=data container} The data
; container to work on.
; @param newinterval {in}{required}{type=real} The new
; frequency_interval to use in the interpolation.  This must be
; non-zero.  If it has opposite sign from the input frequency
; interval, <a href="dcinvert.html">dcinvert</a> will be used to first
...
; frequency value at the keychan channel.  When not supplied, this
...
; do least squares quadratic fit interpolation.
; @keyword quadratic {in}{optional}{type=boolean} When set
; do a quadratic fit interpolation.
; @keyword spline {in}{optional}{type=boolean} When set do a 
; spline interpolation.
; @keyword ok {out}{optional}{type=boolean} This is set to 1 on
; success, otherwise it is 0 (bad arguments, bad data).
;
; @examples
; <pre>
...
; @uses <a href="dcinvert.html">dcinvert</a>
;
; @version $Id$
;-
