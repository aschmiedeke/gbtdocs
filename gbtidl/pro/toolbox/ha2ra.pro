...
; (RA) in degrees using the given LST in seconds. 
; 
; The returned value (RA) is always a postive number between 0 and
; 360.  This works for vectors (lst must either be a single scalar or
; a vector of the same length as the HA argument).
;
; :Params:
;   ha : in, required, type=floating point
...
;       vector, it must have the same number of elements as ha.
;
; :Returns:
;   RA (right ascension) in degrees. The returned value is always
;   between 0.0 and 360.0. Returns NaN on error (missing arguments 
;   or bad number of elements in lst).
;
;-
