...
; to flag and then enough spaces are appended to the input string to
; pad it out to the desired width.  If the input string is wider than
; the desired width, after trimming, it is truncated so that the
; returned string always is of the desired width.
;
; :Params:
;   in : in, required, type=string
;       The input string to pad.
; 
;   width : in, required, type=integer
;       The desired width.  Must be a positive integer.
; 
;   flag : in, optional, type=integer, default=0
;       This is passed directly to strtrim to control on in is trimmed
;       prior to padding. If flag is 0, trailing whitespace is trimmed,
;       if flag is 1, leading whitepsace is trimed, and if flag is 2, 
;       both are trimmed.
;
; :Returns:
;   the padded string
