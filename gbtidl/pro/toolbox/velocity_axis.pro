...
;       The velocity definition to use, chosen from 'RADIO', 'OPTICAL',
;       and 'TRUE'.  If not supplied, the data.velocity_definition value 
;       will be used.
;
;   frame : in, optional, type=string
;       The reference frame to use. If not supplied, the data.velocity_definition
...
;       The actual rest frame used in constructing the velocity axis. The only way 
;       this will not equal the frame argument is if that argument was invalid. In
;       that case, this keyword will be the same as the frame in data.velocity_definition.
;
;   true_veldef : out, optional, type=string
;       The actual velocity frame used in constructing the velocity axis. The only way 
;       this will not equal the frame argument is if that argument was invalid. In that
;       case, this keyword will be the same as data.velocity_definition.
;
; :Returns:
;   A vector of velocities (m/s).  May also set the keywords. Returns -1 on a severe
;   error.  If the veldef or ref_frame are invalid this will use the values found in 
;   the data structure and the values of the true_veldef and true_frame keywords will
;   be set.
;
; :Uses:
;   :idl:pro:`chantovel`
