...
;  (none are required):
; <UL>
; <LI>.VALUE - the starting parameter value (but see the START_PARAMS
;              parameter for more information).
;  
; <LI>.FIXED - a boolean value, whether the parameter is to be held
;              fixed or not.  Fixed parameters are not varied by
;              MPFIT, but are passed on to MYFUNCT for evaluation.
;  
; <LI>.LIMITED - a two-element boolean array.  If the first/second
;                element is set, then the parameter is bounded on the
;                lower/upper side.  A parameter can be bounded on both
;                sides.  Both LIMITED and LIMITS must be given
;                together.
;
; <LI>.LIMITS - a two-element float or double array.  Gives the
;               parameter limits on the lower and upper sides,
;               respectively.  Zero, one or two of these values can be
;               set, depending on the values of LIMITED.  Both LIMITED
;               and LIMITS must be given together.
;  
; <LI>.PARNAME - a string, giving the name of the parameter.  The
;                fitting code of MPFIT does not use this tag in any
;                way.  However, the default ITERPROC will print the
;                parameter name if available.
;  
; <LI>.STEP - the step size to be used in calculating the numerical
;             derivatives.  If set to zero, then the step size is
;             computed automatically.  Ignored when AUTODERIVATIVE=0.
;             This value is superceded by the RELSTEP value.
;
; <LI>.RELSTEP - the *relative* step size to be used in calculating
...
;                step size is chosen.
;
; <LI>.MPSIDE - the sidedness of the finite difference when computing
;               numerical derivatives.  This field can take four
;               values:
; <UL>
; <LI>             0 - one-sided derivative computed automatically
; <LI>             1 - one-sided derivative (f(x+h) - f(x)  )/h
; <LI>            -1 - one-sided derivative (f(x)   - f(x-h))/h
; <LI>             2 - two-sided derivative (f(x+h) - f(x-h))/(2*h)
; </UL>
...
;
; <LI>.MPMAXSTEP - the maximum change to be made in the parameter
...
;  
; <LI>.TIED - a string expression which "ties" the parameter to other
...
;
; <LI>.MPPRINT - if set to 1, then the default ITERPROC will print the
;                parameter value.  If set to 0, the parameter value
;                will not be printed.  This tag can be used to
;                selectively print only a few parameter values out of
;                many.  Default: 1 (all parameters printed)
; </UL>
; @keyword _EXTRA {in}{optional}{type=record} Extra keywords are
; passed to mpcurvefit.
;
